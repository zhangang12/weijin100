...
| R17 | 「正式启动前后端统一开发，选技术栈」 | 定稿技术栈（用户确认 **NestJS / PostgreSQL+Prisma / 阿里云**；Redis+BullMQ / 微信支付V3 / OSS）；产出 `技术方案与开发计划_v1.md`（架构 + **联调约定**金额分/信封/错误码/token2h-30d/快照4h + **Sprint0–5 路线图** + 资源清单 + 决策记录）、`backend/prisma/schema.prisma`（16 表领域模型）、`backend/docker-compose.dev.yml`(本地 pg/redis) + 扩充 `.env.example`；**后端 Express → NestJS 迁移完成并验证**（全局信封拦截器/异常过滤器/前缀 `/api/v1`/CORS、42 接口、**行情真链路保留**、`tsc` 0 报错、`nest build` 通过）；确认免费源**奇数时段**融通金不可取(走兜底)、**RT_PT 铂金未授权**；提交推送。**业务规则主体已定稿，仅留 佣金费率/销售回购溢价口径/铂金代码 3 处可并行补** |
| R18 | 「评估前人 yudao(Java) 代码 + 给 dev 微信号 + 继续开发到全部完成」 | 评估 yudao(芋道 Spring-Boot) → 后端仍 NestJS，借鉴入模型(Listing 加 images/batchNumber/sourcePlace)；取 dev 微信号 AppID `wxa4c8259e43a47829`/Secret(yudao 测试号,仅 dev)；定**架构 v2**(单 ECS docker-compose + 文件本地落盘取消 OSS + 驱动抽象可扩展)；本机 `embedded-postgres` 起 PG(无 sudo/brew) → Prisma 迁移 16 表 + 种子。**后端 Sprint0–4 真库实现+逐 Sprint 验证+提交**：①基建(Prisma/AuthCore-JWT/infra 驱动 wechat·storage·sms/BizException/脱敏) ②微信登录+我的+保证金(冻结/解冻/扣罚,分)+级别 ③挂单浏览/发布+地址+提醒 ④**锁价闭环**(快照A3+原子扣库存A4+冻结C5+订单4h+双方确认完成解冻+成交数+1)+订单/仲裁 ⑤违约/申诉+违约判定服务。每 Sprint `tsc` 0 报错 + node 脚本验证(含两方锁价→确认→解冻对账)。**剩**：微信支付(代交接/充值真扣款)、违约自动判定/24h自动完成(BullMQ+Redis)、实名OCR、正式行情IP授权、docker-compose.prod+Nginx+备份 — 多为外部依赖/资源 |
...
| `项目记忆_2026-07_联调测试与运维.md` | **本轮会话完整记忆**：联调/测试/CI 全程 + 运维手册 + 踩坑清单 | 接手的 Claude |
...
1. 🔴 **合规与资质**（硬前置）：贵金属/金融类目审核、ICP 备案、微信支付商户号、实名核身资质 → 商务/法务。
   - **客户张维钧 2026-07-12 提出的调研（已完成 → 《合规与上架调研_2026-07.html》，可给客户看）**：
     > ⚠️ **最关键结论**：深圳金融委 2026-02-12 已明令禁止「缴纳保证金锁定重量黄金价格、按价差结算」的黄金交易，并禁止通过小程序/APP/网站开展、禁止开发兜售此类软件——与本项目「保证金+锁价」机制**字面高度重合**。可辩护点是「实物交割≠价差结算」，但**须先做法律合规评估**再谈上架。另央行银发〔2025〕124 号要求贵金属从业机构履行反洗钱义务。详见 HTML 文档。原 5 项：
